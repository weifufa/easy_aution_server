<template>
  <div class="mod-config">
    ...
    <el-table :data="dataList" border v-loading="dataListLoading" @selection-change="selectionChangeHandle"
      ...
      <el-table-column prop="catId" header-align="center" align="center" label="分类id">
      </el-table-column>
      ...
      <el-table-column prop="showStatus" header-align="center" align="center" label="是否显示">
        <template slot-scope="scope">
          <el-switch v-model="scope.row.showStatus" active-color="#13ce66" inactive-color="#ff4949" :active-value="1"
            :inactive-value="0" @change="updateCategoryStatus(scope.row)">
          </el-switch>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    updateCategoryStatus(data) {
      let { catId, showStatus } = data;
      this.$http({
        url: this.$http.adornUrl('/product/category/update/status'),
        method: 'post',
        data: this.$http.adornData({ catId, showStatus }, false)
      }).then(({ data }) => {
        this.$message({
          type: "success",
          message: "状态更新成功"
